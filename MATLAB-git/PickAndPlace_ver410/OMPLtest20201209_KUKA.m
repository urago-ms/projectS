...
if (clientID>-1)
    ...
    for count_overall = 1:rep_overall
        ...
        for count = 1:rep
            ...
            fposition1 = [-0.36,    0.15,  0.75,    0,  0,  0];    % [x, y, z, alpha, beta, gamma] first position
            %         fposition2 = [0.2,    0,      0.9,    0,  0,  0];
            fposition3 = [0, 0.9, 0.75,	0,	0,	0];    % above place position
            fposition4 = [0, 0.9, 0.625,	0,	0,	0];   % place position
            fposition5 = [tab_pos_2(1),	tab_pos_2(2),   0.65,    0,  0,  0];    % above pickup position
            %         fposition5 = [tab_pos_2(1),	tab_pos_2(2),   tab_pos_2(3)+0.1,    0,  0,  0];    % above pickup position
            fposition6 = [tab_pos_2(1),	tab_pos_2(2),   tab_pos_2(3)+0.13,    0,  0,  0];    % pickup position
            ...
            % % %               Retrieve the poses (i.e. transformation matrices, 12 values, last row is implicit) of some dummies in the scene
            [res, retInts, target1Pose, retStrings, retBuffer] = sim.simxCallScriptFunction(clientID, ...
                'remoteApiCommandServer', ...
                sim.sim_scripttype_childscript, ...
                'getObjectPose', ...
                [Dum1_handle],[], ...
                ...
                sim.simx_opmode_oneshot_wait);
            % % %               Retrieve the poses (i.e. transformation matrices, 12 values, last row is implicit) of some dummies in the scene
            [res, retInts, target2Pose, retStrings, retBuffer] = sim.simxCallScriptFunction(clientID, ...
                'remoteApiCommandServer', ...
                sim.sim_scripttype_childscript, ...
                'getObjectPose', ...
                [Dum2_handle],[], ...
                ...
                sim.simx_opmode_oneshot_wait);
            % % %               Retrieve the poses (i.e. transformation matrices, 12 values, last row is implicit) of some dummies in the scene
            [res, retInts, target3Pose, retStrings, retBuffer] = sim.simxCallScriptFunction(clientID, ...
                'remoteApiCommandServer', ...
                sim.sim_scripttype_childscript, ...
                'getObjectPose', ...
                [Dum3_handle],[], ...
                ...
                sim.simx_opmode_oneshot_wait);
            % % %               Retrieve the poses (i.e. transformation matrices, 12 values, last row is implicit) of some dummies in the scene
            [res, retInts, target0Pose, retStrings, retBuffer] = sim.simxCallScriptFunction(clientID, ...
                'remoteApiCommandServer', ...
                sim.sim_scripttype_childscript, ...
                'getObjectPose', ...
                [Dum0_handle],[], ...
                ...
                sim.simx_opmode_oneshot_wait);
            % % %               Retrieve the poses (i.e. transformation matrices, 12 values, last row is implicit) of some dummies in the scene
            [res, retInts, target4Pose, retStrings, retBuffer] = sim.simxCallScriptFunction(clientID, ...
                'remoteApiCommandServer', ...
                sim.sim_scripttype_childscript, ...
                'getObjectPose', ...
                [Dum4_handle],[], ...
            ...
            inFloats = horzcat(robotInitialState, target3Pose, approachVector);
            ...
            if (res==0) && length(path)>0
                ...
                inInts = [rob_handle, collisionChecking, minConfigsForIkPath, minConfigsForPathPlanningPath, maxConfigsForDesiredPose, maxTrialsForConfigSearch, searchCount]
                inFloats = horzcat(robotCurrentConfig, target1Pose, approachVector, approachVector2);
