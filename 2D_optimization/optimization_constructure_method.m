...
% best palette position and manipulator position of 9 neighbourhood
best_p_pos = zeros(9,2);
best_m_pos = zeros(9,2);
...
%         if (p_pos_x - c_pos_x)^2 + (p_pos_y - c_pos_y)^2 < (2*(link_1 + link_2))^2
            ...
            if (m_pos_x - p_pos_x)^2 + (m_pos_y - p_pos_y)^2 < (link_1 + link_2)^2 &&  ((m_pos_x - p_pos_x)^2 + (m_pos_y - p_pos_y)^2 > m_min_range^2)
                ...
                % Whether to overlap with the palette(manipulator)
                if (m_pos_x > (p_pos_x + 1/2*pl + 1/2*ml)) || (m_pos_x < (p_pos_x - 1/2*pl - 1/2*ml))   % If it does not overlap in the X-axis direction
                    % Whether to overlap with the conveyor
                    if (p_pos_x > (c_pos_x + 1/2*cl_s + 1/2*ml)) || (p_pos_x < (c_pos_x - 1/2*cl_s - 1/2*ml))   % If it does not overlap in the X-axis direction
                        p_pos_x;
                        p_pos_y;
                        %                     break;
                    elseif (p_pos_y > (c_pos_y + 1/2*cl_l + 1/2*ml)) || (p_pos_y < (c_pos_y - 1/2*cl_l - 1/2*ml))   % If it does not overlap in the Y-axis direction
                        p_pos_x;
                        p_pos_y;
                        %                     break;
                    else
                ...
                elseif (p_pos_y > (p_pos_y + 1/2*pl + 1/2*ml)) || (p_pos_y < (p_pos_y - 1/2*pl - 1/2*ml))   % If it does not overlap in the Y-axis direction
                    % Whether to overlap with the conveyor
                    if (p_pos_x > (c_pos_x + 1/2*cl_s + 1/2*ml)) || (p_pos_x < (c_pos_x - 1/2*cl_s - 1/2*ml))   % If it does not overlap in the X-axis direction
                        p_pos_x;
                        p_pos_y;
...
%         end
        ...
        if constraint_flag == 0
            ...
            %%%%% Best Palette Position and Manipulator Position of 9 Neighbourhood %%%%%%%%%%%%
            best_p_pos(neighbour_num,1) = p_pos_x;
            best_p_pos(neighbour_num,2) = p_pos_y;
            
            best_m_pos(neighbour_num,1) = m_pos_x;
            best_m_pos(neighbour_num,2) = m_pos_y;
            ...
            %%%%% Best Palette Position and Manipulator Position of 9 Neighbourhood %%%%%%%%%%%%
            best_p_pos(neighbour_num,1) = 9999;
            best_p_pos(neighbour_num,2) = 9999;
            
            best_m_pos(neighbour_num,1) = 9999;
            best_m_pos(neighbour_num,2) = 9999;
        end
